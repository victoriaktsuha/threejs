...
scene.add(light);
// ! Affects objects using MeshLambert, MeshStandar, MeshPhysical and MeshPhongMaterial
...
// light.target = torus[1];
// ! we can set a target for the light, from where this light will come
light.target.position.set(5, 10, -10); // ! we can use a position as target, but in this case, we need to add at the scene
...
function animate() {
  ...
  helper.update();
  // ! used to update when the light helper has a target
