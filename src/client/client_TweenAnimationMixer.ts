...
light1.angle = Math.PI / 8; // ! able just with SpotLight()
light1.penumbra = 0.5; // ! able just with SpotLight()
...
light2.angle = Math.PI / 8; // ! able just with SpotLight()
light2.penumbra = 0.5; // ! able just with SpotLight()
...
function onDoubleClick(event: MouseEvent) {
  ...
  if (intersects.length > 0) {
    ...
    setAction(animationActions[3]); // ! action while object is moving from a point to another
    ...
    new TWEEN.Tween(modelMesh.position)
      ...
      .onUpdate(() => {
        ...
        // ! lights follow the object
        light1.target = modelMesh;
        light2.target = modelMesh;
      })
      .start()
      .onComplete(() => setAction(animationActions[2])); // ! action when the object stops moving
