  ...
  function (gltf) {
    gltf.scene.traverse(function (child) {
      if ((child as THREE.Mesh).isMesh) {
        ...
        m.castShadow = true;
        (m.material as THREE.MeshStandardMaterial).flatShading = true; // !  give a
        sceneMeshes.push(m);
      }
...
function onMouseMove(event: MouseEvent) {
  ...
  //   console.log(mouse); // ! While move the mouse in the scene, we'll get the current coordinates
  ...
  if (intersects.length > 0) {
    ...
    // ! Configure the helper line according to the pointer position
...
function onDoubleClick(event: MouseEvent) {
  ...
  if (intersects.length > 0) {
    ...
    cube.lookAt(n);
    cube.rotateX(Math.PI / 2); // ! allow the material to be placed according to the angle
    cube.position.copy(intersects[0].point);
    cube.position.addScaledVector(n, 0.1);

    scene.add(cube);
    sceneMeshes.push(cube); // ! allow cubes being piled up/stacked up
  }
}
