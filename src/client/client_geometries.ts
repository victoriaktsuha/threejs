...
cubeRotationFolder.add(cube.rotation, "x", 0, Math.PI * 2, 0.01); // ! Display controls in the page, in this case, to rotation on x axle, delivering 0 as minimun and Math.PI * 2 (= 360) as maximum
...
cubeScaleFolder
  .add(cube.scale, "x", -5, 5)
  .onFinishChange(() => console.dir(cube.geometry));
cubeScaleFolder.add(cube.scale, "y", -5, 5);
cubeScaleFolder.add(cube.scale, "z", -5, 5);
cubeFolder.add(cube, "visible", true); // ! Gives to the browser controls an option to enable/disable the object visibility
cubeFolder.open(); // ! Define the tab open once the page loads
