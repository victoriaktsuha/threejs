...
const controls = new DragControls(cubes, camera, renderer.domElement); // ! 'cubes' needs to be an array - in case of just an object, embrace it with []
controls.addEventListener("dragstart", function (event) {
  event.object.material.opacity = 0.33; // ! decrease opacity when mouse is holding the object
});
controls.addEventListener("dragend", function (event) {
  event.object.material.opacity = 1; // ! increase back opacity when mouse drop object
});
...
function animate() {
  ...
  //controls.update() // ! DragControls doesn't need .update()
